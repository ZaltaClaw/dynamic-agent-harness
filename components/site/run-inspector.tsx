...
function tone(type: string, artifactReady: boolean) {
  if (type === "run.completed") return artifactReady ? "bg-green" : "bg-ink-3";
    ...
    <div className="flex min-h-0 flex-1 flex-col">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto p-4">
        <div className="flex items-center justify-between gap-3">
          ...
          <span className="inline-flex h-6 items-center gap-1.5 rounded-full bg-inset px-2 text-[10px] font-medium text-ink-2 shadow-hairline"><span className={`size-1.5 rounded-full ${status === "completed" ? artifactPath ? "bg-green" : "bg-ink-3" : status === "failed" ? "bg-red" : status === "waiting_for_approval" ? "bg-orange" : "bg-accent"}`} style={status === "running" ? { animation: "pulse 1.3s ease-in-out infinite" } : undefined}/>{status.replaceAll("_", " ")}</span>
        </div>
        ...
        {artifactPath && (
          <button type="button" onClick={onCopyArtifact} className="mt-4 flex w-full items-center gap-2 rounded-card bg-green-tint p-3 text-left shadow-hairline transition-colors hover:brightness-95">
            ...
            <span className="min-w-0"><span className="block text-[11.5px] font-medium text-green-ink">Artifact written</span><span className="mt-0.5 block truncate font-mono text-[9.5px] text-green-ink">{artifactPath}</span></span>
          </button>
        )}
          ...
          <div className="relative mt-2 pl-4 before:absolute before:top-3 before:bottom-3 before:left-[3px] before:w-px before:bg-line">
            {events.map((event) => (
              <div key={event.id} className="relative border-b border-line py-2.5 last:border-b-0">
                <span className={`absolute top-4 -left-[15px] size-1.5 rounded-full ring-2 ring-page ${tone(event.type, Boolean(artifactPath))}`} />
